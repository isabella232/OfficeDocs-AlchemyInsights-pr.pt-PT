---
title: Organize o seu e-mail no Outlook na web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: a6f7fd08098a7d56351faa56c2b05878d99a8ee3
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 04/22/2020
ms.locfileid: "43742852"
---
# <a name="organize-your-email"></a>Organize o seu e-mail

No Outlook na web, pode utilizar comandos como **Sweep** e **Move para** apagar ou mover automaticamente mensagens de um determinado remetente. Estes comandos aparecem acima da lista de mensagens quando seleciona uma mensagem na sua caixa de entrada.

- **Varrer** <img src='data:image/png;base64,' />
 permite-lhe apagar rapidamente as mensagens na sua caixa de entrada. Pode eliminar automaticamente todos os e-mails de entrada desse remetente, guardar apenas o mais recente e-mail e apagar o resto, ou apagar e-mails com mais de 10 dias.
- **Mover-se para** <img src='data:image/png;base64,' /> permite-lhe mover uma mensagem específica para outra pasta.

Para saber mais, consulte Trabalhar com pastas de [mensagens no Outlook na web](https://support.office.com/article/ae0f10d6-54e7-4f29-acd3-78cdc3fdcb9f).