---
title: Testar imagens inline de Alquimia
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: ea91a0ef0e003d1a2444d4d5e72ccbdd608bf102
ms.sourcegitcommit: 1d98db8acb9959aba3b5e308a567ade6b62da56c
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 08/22/2019
ms.locfileid: "36507174"
---
# <a name="access-the-domains-page"></a>Aceder à página de domínios

Para aceder à página de domínios, basta clicar neste ícone abaixo:

  
<img src='data:image/png;base64,'/>

Em seguida, pode adicionar, modificar ou remover o domínio.
Para mais informações, consulte o [FAQ de domínios.](https://support.office.com/article/1272bad0-4bd4-4796-8005-67d6fb3afc5a.aspx)
  