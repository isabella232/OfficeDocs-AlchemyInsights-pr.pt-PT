---
title: Testando imagens em linha de alquimia
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: 037331d71f06750d972c0b6278b23bb15c4806ca
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 10/18/2019
ms.locfileid: "36743140"
---
# <a name="access-the-domains-page"></a>Acessar a página domínios

Para acessar a página de domínios, basta clicar neste ícone abaixo:

  
<img src='data:image/png;base64,'/>

Em seguida, você pode adicionar, modificar ou remover seu domínio.
Para obter mais informações, consulte a [FAQ de domínios.](https://docs.microsoft.com/office365/admin/setup/domains-faq)
  