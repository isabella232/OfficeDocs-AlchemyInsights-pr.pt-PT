---
title: Testando imagens em linha de alquimia
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: 0b06093dabd685f76cc39b1d7c0f8b03883b6e79
ms.translationtype: MT
ms.contentlocale: pt-PT
ms.lasthandoff: 10/25/2019
ms.locfileid: "36743140"
---
# <a name="access-the-domains-page"></a><span data-ttu-id="bb652-102">Acesse a página de domínios</span><span class="sxs-lookup"><span data-stu-id="bb652-102">Access the Domains Page</span></span>

<span data-ttu-id="bb652-103">Para acessar a página de domínios, basta clicar neste ícone abaixo:</span><span class="sxs-lookup"><span data-stu-id="bb652-103">To access the domains page, just click this icon below:</span></span>

  
<img src='data:image/png;base64,'/>

<span data-ttu-id="bb652-104">Em seguida, você pode adicionar, modificar ou remover seu domínio.</span><span class="sxs-lookup"><span data-stu-id="bb652-104">Then you can add, modify, or remove your domain.</span></span>
<span data-ttu-id="bb652-105">Para mais informações, consulte o [Domains FAQ.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span><span class="sxs-lookup"><span data-stu-id="bb652-105">For more information, see the [Domains FAQ.](https://docs.microsoft.com/office365/admin/setup/domains-faq)</span></span>
  